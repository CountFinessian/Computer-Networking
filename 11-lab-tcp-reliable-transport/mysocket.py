...
class TCPSocket(TCPSocketBase):
    ...
    def send_if_possible(self) -> int:
        ...
        while (self.send_buffer.bytes_outstanding() < self.cwnd):
            data, seqNumber = self.send_buffer.get(self.mss)

            if (data == b''):
                break
            self.send_packet(seqNumber, 0, 0, data)
    ...
    def handle_data(self, pkt: bytes) -> None:
        ...
        recievedData, recievedSeq = self.receive_buffer.get()

        self.ack = self.receive_buffer.base_seq
        self.send_ack()

        if (recievedData != b''):
            self.ready_buffer += recievedData
            self._notify_on_data()
        ...
        if self.fast_retransmit:
            ...
            if tcp_hdr.ack == self.last_ack:
                ...
                if self.num_dup_acks == 3:
                    self.retransmit()
            elif (tcp_hdr.ack > self.last_ack):
                self.num_dup_acks = 0
    ...
    def retransmit(self) -> None:
        data, seqNumber = self.send_buffer.get_for_resend(self.mss)
