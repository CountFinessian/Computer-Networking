...
class UDPSocket:
    ...
    def handle_packet(self, pkt: bytes) -> None:
        ...
        # data, remote_addr, remote_port
        self.buffer.append((data, IPClass.src, UDPClass.sport))
        self._notify_on_data()

    @classmethod
    def create_packet(cls, src: str, sport: int, dst: str, dport: int,
            data: bytes=b'') -> bytes:
        UDPLen = UDP_HEADER_LEN + len(data)
        IPLen = UDPIP_HEADER_LEN + len(data)
        UDP = UDPHeader(sport, dport, UDPLen, 0)
        IP = IPv4Header(IPLen, 64, IPPROTO_UDP, 0, src, dst)
        packet = IP.to_bytes() + UDP.to_bytes() + data
        return packet
...
class TCPSocket(TCPSocketBase):
    ...
    def handle_syn(self, pkt: bytes) -> None:
        TCPClass, IPClass = self.return_TCP_IP_Classes(pkt)

        if (TCPClass.flags & TCP_FLAGS_SYN):
            self.base_seq_other = TCPClass.seq
            ack = TCPClass.seq + 1
            self.send_packet(self.base_seq_self, ack, TCP_FLAGS_ACK | TCP_FLAGS_SYN)
            self.state = TCP_STATE_SYN_RECEIVED

    def handle_synack(self, pkt: bytes) -> None:
        TCPClass, IPClass = self.return_TCP_IP_Classes(pkt)
        flags = TCPClass.flags
        ack = TCPClass.ack

        if (flags & TCP_FLAGS_SYN) and (flags & TCP_FLAGS_ACK) and (ack == self.base_seq_self + 1):

            self.base_seq_other = TCPClass.seq
            ack = TCPClass.seq + 1
            self.send_packet(self.base_seq_self + 1, ack, TCP_FLAGS_ACK)
    ...
    def handle_ack_after_synack(self, pkt: bytes) -> None:
        ...
        if not (flags & TCP_FLAGS_SYN) and (flags & TCP_FLAGS_ACK) and (ack == self.base_seq_self + 1):
            self.state = TCP_STATE_ESTABLISHED

    def return_TCP_IP_Classes(self, pkt):
        IPHdr = pkt[:IP_HEADER_LEN]
    ...
    def create_packet(cls, src: str, sport: int, dst: str, dport: int,
        ...
        IPLen = TCPIP_HEADER_LEN + len(data)
        TCP = TCPHeader(sport, dport, seq, ack, flags, 0)
        IP = IPv4Header(IPLen, 64, IPPROTO_TCP, 0, src, dst)
        packet = IP.to_bytes() + TCP.to_bytes() + data
        return packet

    def send_packet(self, seq: int, ack: int, flags: int,
            data: bytes=b'') -> None:
        packet = self.create_packet(self._local_addr, self._local_port, self._remote_addr, self._remote_port, seq, ack, flags, data)
        self._send_ip_packet(packet)
