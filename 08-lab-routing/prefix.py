...
def ip_prefix_mask(family: int, prefix_len: int) -> int:
    ...
    totalBits = numberOfBits(family)

    totalPrefix = all_ones(totalBits)
    prefixReduction = all_ones(totalBits - prefix_len)
    offset = totalPrefix - prefixReduction
...
def ip_prefix(address: int, family: int, prefix_len: int) -> int:
    ...
    prefixMaskInt = ip_prefix_mask(family, prefix_len)
    return address & prefixMaskInt
...
def ip_prefix_total_addresses(family: int, prefix_len: int) -> int:
    ...
    prefixReduction = numberOfBits(family) - prefix_len
    prefixReduction = 2 ** prefixReduction
    return prefixReduction
...
def ip_prefix_nth_address(prefix: int, family: int,
    ...
    adjustedPrefix = ip_prefix(prefix, family, prefix_len)
    return adjustedPrefix + n
...
def ip_prefix_last_address(prefix: int, family: int, prefix_len: int) -> int:
    ...
    adjustedPrefix = ip_prefix(prefix, family, prefix_len)
    # print(f"adjusted prefix {hex(adjustedPrefix)}")
    lastAddress = all_ones(numberOfBits(family) - prefix_len)
    # print(f"adjustedLast address {hex(lastAddress)}")

    return adjustedPrefix + lastAddress

def numberOfBits(family: int):
    totalBits = 0
...
class Prefix:
    ...
    def __contains__(self, address: str) -> bool:
        ...
        # find out if the prefix matches the address
        prefix = self.prefix
        prefixLength = self.prefix_len
        addressInt = ip_str_to_int(address)
        prefixedAdress = ip_prefix(address, family, prefixLength)

        if (prefixedAdress == prefix):
            # print(f"prefixedAddress {prefixedAdress}")
            # print(f"prefix {prefix}")
            largestAdress = ip_prefix_last_address(prefix, family, prefixLength)
            
            if (addressInt - largestAdress) <= 0:
                return True
