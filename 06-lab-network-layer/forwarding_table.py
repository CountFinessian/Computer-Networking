...
class ForwardingTable(object):
    ...
    def get_entry(self, address: str) -> tuple[str, str]:
        ...
        matchingPrefixValues = None
        largestMatchingPrefix = -1

        for prefix in self.entries:
            if address in prefix:
                prefixLength = prefix.prefix_len
                if (prefixLength > largestMatchingPrefix):
                    largestMatchingPrefix = prefixLength
                    matchingPrefixValues = self.entries[prefix]

        if (largestMatchingPrefix != -1):
            return matchingPrefixValues
        else:
            return None, None
